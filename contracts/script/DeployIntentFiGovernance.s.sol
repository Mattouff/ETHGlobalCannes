...
contract DeployIntentFiGovernance is Script {
    ...
    function run() external {
        ...
        IntentFiGovernance governance = new IntentFiGovernance(
            config.governanceToken,
            config.owner,
            config.emergencyMultisig
        );
    ...
    function _verifyDeployment(IntentFiGovernance governance, ChainConfig memory config) internal view {
        ...
        require(
            address(governance.governanceToken()) == config.governanceToken,
            "Governance token mismatch"
        );
        ...
        for (uint256 i = 0; i < config.emergencyMultisig.length; i++) {
            require(
                governance.emergencyMultisig(config.emergencyMultisig[i]),
                "Emergency multisig not set"
            );
        }
